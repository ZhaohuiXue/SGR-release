...
%% ground truth image
load CASI_SASI_24_trg;
img_gt = reshape(img_trg,sz(1)*sz(2),1);
...
per_class_all = [5];%for dictionary
no_classes = 8;

show_CMap = 'yes'; % yes | no
mu = 6;        % MLL soomthness parameter

lambda_all = 0.00001;%[1e-10 1e-8 1e-6 1e-4 1e-2 1];
gamma_all  = 1;%[1e-2 1e-1 1 1e1 1e2 1e3];

%% Generate neighborhood
classifier_type_all=cellstr(strvcat('SGR'));
neighorhood_method_all = cellstr(strvcat('noneighbor'));%noneighbor (SGR) | fix | fix-weighted (WSGR)
                ...
                for i_classifier_type=1:length(classifier_type_all)
                    ...
                    for i_sample_size = 1:length(per_class_all)
                        ...
                        for i_MC = MMiter
                            ...
                            output_folder = sprintf('%s%s%s%s%s',pwd,'\results_CASI_SASI\',...
